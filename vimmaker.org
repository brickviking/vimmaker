...
I have also included a bash autocompletion script, although it will rely on the user having some
completion scripts in place already as well as the bash-completion package installed in whatever
...
versions earlier than 8. That rather depends on how far the commit history goes back. In addition,
we'll make a note about the separate hg source tree, as this script doesn't process it any further
than merely distcleaning and fetching. We don't configure/compile that tree nor install it, as it's
a mirror of the github canonical source.
...
# TODO: add compile-side for the mercurial mirror. We don't have it currently.
#+END_SRC
