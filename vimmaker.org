...
The boilerplate help just in case I forgot what switches I coded. Uses UNIX syntax (-x) not Microsoft (/x). There's also a slight bug with VIMCONFIGPARAMS
...
To be frank, it doesn't care where it's put, as long as it can be called from somewhere on the user's
PATH.
